...
dl="$(curl -s https://api.github.com/repos/valiray/proton-ge-rtsp-cam/releases/latest | grep browser_download_url | cut -d\" -f4 | grep .tar.gz)"
ver="${dl%.tar.gz}"
ver="${ver##*/}"
if [[ -d "$compatdir"/"$ver" ]]; then
	...
	echo -e "Updating Proton GE RTSP Cam..."
fi
...
echo "Download Proton GE RTSP Cam tarball..."
curl --progress-bar -LOJ "$dl"
echo -n "Download Proton GE RTSP Cam checksum... "
curl -sLOJ "$(curl -s https://api.github.com/repos/valiray/proton-ge-rtsp-cam/releases/latest | grep browser_download_url | cut -d\" -f4 | grep .sha512sum)" && echo "Done."
