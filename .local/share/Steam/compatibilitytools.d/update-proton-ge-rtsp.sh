...
# Check if Proton-RTSP needs to be updated
...
dl="$(curl -s https://api.github.com/repos/SpookySkeletons/proton-ge-rtsp/releases/latest | grep browser_download_url | cut -d\" -f4 | grep .tar.gz | head -n 1)"
ver="${dl%.tar.gz}"
ver="${ver##*/}"
...
if [[ $(stat -f -c %T /tmp/) == 'tmpfs' ]] && ( [[ $(free --byte | tr -s " " | cut -d " " -f7 | head -n 2 | tail -n 1) -lt 1073741824 ]] || [[ $(df /tmp/ --output=avail --block-size=1 | tail -n 1) -lt 2147483648 ]] ); then
	...
	dldir="$HOME/.cache/proton-rtsp/"
else
	dldir="/tmp/proton-rtsp"
fi
...
echo "Download Proton RTSP tarball..."
curl --progress-bar -LOJ "$dl"
echo -n "Download Proton RTSP checksum... "
...
mkdir -p "$compatdir"
tar -xf *proton*.tar.gz -C "$compatdir" || exit 1
